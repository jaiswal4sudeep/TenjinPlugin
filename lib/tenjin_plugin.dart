...
class TenjinPlugin {
  ...
  static TenjinPlugin instance = TenjinPlugin._();
  ...
  void updatePostbackConversionValueCoarseValue(
    int conversionValue,
    String coarseValue,
  ) {
  ...
  void updatePostbackConversionValueCoarseValueLockWindow(
    int conversionValue,
    String coarseValue,
    bool lockWindow,
  ) {
    _channel
        .invokeMethod('updatePostbackConversionValueCoarseValueLockWindow', {
          'conversionValue': conversionValue,
          'coarseValue': coarseValue,
  ...
  void transactionWithReceipt({
    ...
    if (isValidIOS || isValidAndroid) {
      ...
      debugPrint('TenjinPlugin.instance transaction is missing data');
    }
  }
  ...
  Future<Map<String, dynamic>?> getAttributionInfo() async {
    try {
      ...
      debugPrint("Failed to get attribution info: '${e.message}'.");
      return null;
    } catch (e) {
  ...
  Future<String?> getCustomerUserId() async {
    try {
      ...
      debugPrint("Failed to get user id: '${e.message}'.");
      return null;
    }
  }
  ...
  Future<String?> getAnalyticsInstallationId() async {
    try {
      final String? installationId = await _channel.invokeMethod(
        'getAnalyticsInstallationId',
      );
      return installationId;
    } on PlatformException catch (e) {
      debugPrint("Failed to get analytics installation id: '${e.message}'.");
      return null;
    }
  }
